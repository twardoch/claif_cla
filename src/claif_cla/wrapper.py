...
class ResponseCache:
    ...
    def get(self, prompt: str, options: ClaifOptions) -> Optional[list]:
...
class ClaudeWrapper:
    ...
    async def query(
        ...
        if "timeout" in str(last_error).lower():
            raise TimeoutError(f"Claude query timed out after {self.retry_count} retries")
        else:
            raise ProviderError("claude", f"Query failed after {self.retry_count} retries", {
