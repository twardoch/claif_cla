...
def install_claude_bundled(install_dir: Path, dist_dir: Path) -> bool:
    ...
    try:
        ...
        # Create a platform-appropriate wrapper script in the main install_dir
        # This allows calling 'claude' directly from PATH.
        if platform.system() == "Windows":
            # Create Windows batch wrapper
            wrapper_path: Path = install_dir / "claude.cmd"
            if wrapper_path.exists():
                logger.info(f"Removing existing Claude wrapper script: {wrapper_path}")
                wrapper_path.unlink()

            wrapper_content: str = f"""@echo off
REM Claude CLI wrapper script
REM Generated by claif install

REM Change to Claude directory and run the executable
cd /d "{claude_install_dir}" && claude.exe %*
if errorlevel 1 (
    cd /d "{claude_install_dir}" && claude %*
)
"""
            wrapper_path.write_text(wrapper_content)

            # Also create PowerShell wrapper for better compatibility
            ps_wrapper_path: Path = install_dir / "claude.ps1"
            ps_wrapper_content: str = f"""# Claude CLI PowerShell wrapper script
# Generated by claif install

try {{
    Push-Location "{claude_install_dir}"
...
"""
            ps_wrapper_path.write_text(ps_wrapper_content)
            logger.info(f"Created Windows wrapper scripts: {wrapper_path} and {ps_wrapper_path}")
        else:
            # Create Unix bash wrapper (original behavior)
...
"""
        ...
        logger.success(f"✓ Claude CLI installed successfully at {claude_install_dir} with wrapper at {wrapper_path}")
        return True
...
def install_claude() -> dict[str, Any]:
        ...
        try:
            ...
            if script_path.exists():
                ...
                if result.returncode == 0:
                    logger.success("✓ Claude installed successfully via Windows installer")
    ...
    dist_dir: Path | None = bundle_all_tools()
    if not dist_dir:
        return {
            "installed": [],
            "failed": ["bundling"],
...
def uninstall_claude() -> dict[str, Any]:
    ...
    if uninstall_tool("claude"):
        logger.success("✓ Claude wrapper script uninstalled.")
    ...
    if claude_bin_dir.exists() and claude_bin_dir.is_dir():
        try:
            ...
            logger.success(f"✓ Removed Claude installation directory: {claude_bin_dir}")
